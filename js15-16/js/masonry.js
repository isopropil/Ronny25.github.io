...
jQuery.fn.gridDiv = function(options, load){
    return this.each(function() {
        var msr = $(this);
        if (typeof options == 'object' || !options) {
            lCount = 0, count = msr.find('.gridItem').length, col = 2, mar = 10;
            if (options) {
            ...
            msr.height(0), lCount = 0, count = msr.find('.gridItem').length;
            init();
        } else if (options == 'destroy')
            msr.find('.gridItem').each(function(index, el){
        ...
        function init() {
            ...
            msr.css('position', 'relative').find('.gridItem').each(function(i, el) {
        ...
        function doload() {
            ...
            $.each(load, function(i, el) {
                item = $(msr).append(el).find('.gridItem').last();
